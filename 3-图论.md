...
namespace Dominator {
  int fa[maxn], idx[maxn], tot, ridx[maxn];
  int c[maxn], compress[maxn], semi[maxn], idom[maxn];
  ...
  int fix(int x) {
    if (c[x] == -1) return x;
    int &f = c[x], rt = fix(f);
    if (idx[semi[compress[x]]] > idx[semi[compress[f]]])
      compress[x] = compress[f];
    return f = rt;
  }
  void build(int rt) {
    dfs(rt);
    ...
    for (int i = 1; i <= tot; i++) {
      int x = fa[ridx[i]];
      int sdom = semi[ridx[i]];
      while (idx[x] > idx[sdom]) {
        x = idom[x];
      }
      idom[ridx[i]] = x;
    }
