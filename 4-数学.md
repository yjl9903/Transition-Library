...
线性递推数列 $\{a_1,a_2,\dots,a_n,\dots\}$，有线性递推式 $\{r_0, r_1, \dots, r_{m-1}\}$，满足 $a_n=\sum_{i=1}^{m-1} r_i a_{n-i}$。

构造多项式 $G=x^k-\sum_{i=0}^{m-1} r_{m-i-1} x^i$，求 $x^n \text{ mod }G$。
...
$d(ijk)=\sum_{x|i}\sum_{y|j}\sum_{z|k}[(x,y)=1][(x,z)=1][(y,z)=1]$

$f(n)=\sum_{x|n}[(x,\frac{n}{x})=1]=2^{\omega(n)}=\sum_{i|n}\mu^2(i)$ $\sum_{i=1}^nf(i)=\sum_{i=1}^n\sum_{d|i}\mu^2(d)=\sum_{d=1}^{n}\mu^2(d)\lfloor\frac{n}{d}\rfloor=\sum_{d=1}^n\sum_{i^2|d}\mu(i)\lfloor\frac{n}{d}\rfloor=\sum_{i=1}^{\sqrt n}\mu(i)\sum_{d=1}^{\lfloor\frac{n}{i^2}\rfloor}\lfloor\frac{\lfloor\frac{n}{i^2}\rfloor}{d}\rfloor$

$f(n)=\sum_{x|n}[(x,\frac{n}{x})]=\sum_{i|n}g(i)$，$g(i)$ 为 $i$ 的最大平方根

$\sum_{i=1}^nf(i)=\sum_{i=1}^n\sum_{d|i}g(d)=\sum_{d=1}^{n}g(d)\lfloor\frac{n}{d}\rfloor=\sum_{d=1}^n\sum_{i^2|d}\phi(i)\lfloor\frac{n}{d}\rfloor=\sum_{i=1}^{\sqrt n}\phi(i)\sum_{d=1}^{\lfloor\frac{n}{i^2}\rfloor}\lfloor\frac{\lfloor\frac{n}{i^2}\rfloor}{d}\rfloor$

$p|C(n,k)$，当且仅当 $n$ 与 $k$ 在 $p$ 进制下作减法发生借位。

$ix \equiv jx(mod),(x,mod)=1$，则 $i\equiv j(mod)$。
...
$\sum_{i=r}^nC(i,r)=C(n+1,r+1)$

### 第一类斯特林数

组合含义：$s(n,k)$表示把$n$个不同数分成$k$组，每组是一个环的方案数

递推式： $s(n,k)=(n-1)s(n-1,k)+s(n-1,k-1)$

生成函数：$f(x)=x(x+1)\dots(x+n-1)$，可以 $O(n\log n)$ 求 $s(n,1),s(n,2)\dots s(n,k)$

性质：$\sum_{i=0}^n s(n,i)=n!$

+ $s(n,n-k)$ 是一个关于 $n$ 的至多 $2k$ 次多项式，可以 $O(k^2)$ 求 $s(n,n-k)$；

+ $1$ 到 $n$ 所有 $k$ 个不同数的乘积和为关于 $n$ 的至多 $2k$ 次多项式,生成函数为 $f(x)=(x+1)(2x+1) \dots (nx+1)$，所以答案为 $s(n+1,n+1-k)$。

### 第二类斯特林数

组合含义：$S(n,k)$ 表示把 $n$ 个不同数划分成 $k$ 个非空子集的方案数

递推式：$S(n,k)=kS(n-1,k)+S(n-1,k-1)$
...
令其所有根为 $x_1,x_2,\dots,x_n$,

设 $b_i=a_{n-i}$(反位)，$S_k=\sum_{i=1}^{n}x_i^k$,

恒有
$$
\sum_{i=1}^{k}S_ib_{k-i}+kb_k=0,k\in N^*
$$
经典应用：给定数组$a_1,a_2,\dots,a_n$，对于所有 $1\le k \le n$，求 $S_k=\sum_{i=1}^na_i^k$。

### 高斯函数

$[n \text{ is odd}]=n-\lfloor \frac{n}{2}\rfloor \times 2$

$\lfloor x\rfloor+\lfloor x+ \frac{1}{m} \rfloor+...\lfloor x+ \frac{m-1}{m} \rfloor=\lfloor mx\rfloor$

另上式 $x=\frac{n}{p},m=p$，则 $\lfloor \frac{n}{p}\rfloor+\lfloor \frac{n+1}{p}\rfloor+...\lfloor \frac{n+p-1}{p} \rfloor=n$。

### 勾股方程

$a^2+b^2=c^2$，$(a,b,c)=1$，称为本原股解。

本原勾股解通解为 $a=st,b=\frac{t^2-s^2}{2},c=\frac{t^2+s^2}{2}$，其中 $s\lt t$，$(s,t)=1$，$s,t$ 为奇数。

一般勾股解通解为 $a=kst,b=k\frac{t^2-s^2}{2},c=k\frac{t^2+s^2}{2}$，其中 $s\lt t$，$(s,t)=1$，$s,t$ 为奇数，$k\in Z^+$。

### 划分数

生成函数 $F(x)=\prod_{i=1}^{\infin}(1+x^i+x^{2i}+...)=\prod_{i=1}^{\infin} \frac{1}{1-x^i}$。

$\phi(x)=\prod_{i=1}^{\infin}(1-x^i)=1+\sum_{i=-\infin,i\ne0}^{\infin}(-1)^ix^{\frac{i(3i-1)}{2}}=1+\sum_{i=1}^{\infin}(-1)^ix^{\frac{i(3i\pm1)}{2}}$。

### 其他

$[1,n]$ 内 $\text{lowbit}(i)=x$ 的数量有 $(x\le n)+(n-x)/(2\cdot x)$。

$k$ 进制下数字根 = $(\text{sumdigit}-1) \bmod (k-1)+1$。
...
$$
\sum^{n}_{i=1}i^k = \frac{1}{k+1}\sum^{k+1}_{i=1}{k+1 \choose i} B_{k+1-i}(n+1)^i \\
B_0 = 1\\
\sum^n_{k=0}{n+1\choose 1}B_k = 0\\
B_n = -\frac{1}{n+1}\sum^{n-1}_{i=0}{n+1 \choose i}B_i
$$
...
int main() {
    ...
    while (cin >> n >> k) {
        ll ans = 0;
        for (int i = 1; i <= k + 1; i++) {
            ans = (ans + C[k + 1][i] * B[k + 1 - i] % mod * qpow(n + 1, i)) % mod;
        }
        ans = ans * inv(k + 1) % mod;
        cout << ans << endl;
    }
...
Burnside：$\frac{\sum_{G \in F} C(G)}{|F|}$

记置换 $G$ 的不动点数量为 $C(G)$，那么所有等价类的数量等于 $C(G)$ 的平均值。

Polya：$\frac{\sum_{G \in F} m^{k(G)}}{|F|}$，即快速计算 Brunside 不动点数。
...
int main() {
    ...
    while (T--) {
        ...
        for (int i = 1; i * i <= n; i++) {
            if (n % i) continue;
            sum = (sum + qpow(n, n / i - 1, p) * euler(i)) % p;
            if (i * i != n) {
                sum = (sum + qpow(n, i - 1, p) * euler(n / i)) % p;
            }
        }
        printf("%lld\n", sum);
    }
