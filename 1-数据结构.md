...
2. $ secondMax[l \dots r] < x < max[l \dots r]$ ：将 $max[l \dots r]$ 内的所有最大值覆盖为 $x$；
...
来源：[HDu6430 TeaTree](http://acm.hdu.edu.cn/showproblem.php?pid=6430)。
...
int main(){
  ...
  for (int i = n; i >= 1; i--) {
    f = fa[i];
    root[f] = join(root[i], 1, maxn - 1, root[f]);
  }
