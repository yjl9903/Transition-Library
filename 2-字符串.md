...
来源：[2020 ICPC 小米邀请赛决赛 D Rikka with New Year's Party](https://ac.nowcoder.com/acm/contest/9328/D)。
...
while (scanf("%s", s + 1) == 1) {
    for (int i = sam::sz; i >= 1; i--) dep[i] = 0;
    int u = 1, l = 0;
    for (int i = 1; s[i]; i++) {
        int c = s[i] - 'a';
...
来源：[Tokitsukaze, CSL and Palindrome Game](http://acm.hdu.edu.cn/showproblem.php?pid=6791)。

```c++
struct AP {
  int st, dif, cnt; // 首项，公差，个数
...
+ [H-病毒_2020上海高校程序设计竞赛暨第18届上海大学程序设计联赛夏季赛（同步赛）](https://ac.nowcoder.com/acm/contest/6871/H)；
+ [F-Generating Strings_第 45 届国际大学生程序设计竞赛（ICPC）亚洲区域赛（昆明）（重现赛）](https://ac.nowcoder.com/acm/contest/14055/F)。
